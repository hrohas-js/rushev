<template>
  <div
      class="__content__rows"
      :class="{rows__mobile:$store.state.display_width < 767}"
  >
    <div class="good-img">
      <img
          v-lazy="{src: wishgood.main_pair.detailed.image_path.replace(/\.jpg|\.png/gi, '.webp'), loading: 'data:image/gif;base64,'}"
          alt="good"
      />
    </div>
    <div v-if="$store.state.display_width > 767" class="good-name">
      {{ wishgood.product }}
    </div>
    <div v-if="$store.state.display_width > 767" class="good-price">
      {{ parseInt(wishgood.price) }}
    </div>
    <div v-if="$store.state.display_width < 767" class="mobile-column">
      <div class="name-close">
        <div class="good-name">
          {{ wishgood.product }}
        </div>
        <div
            class="add-to-bascet__close-button"
            @click="removeWishGood(wishgood.product_id)"
        >
          <img src="@/assets/img/svg/deleteWishButton.svg" alt="delete">
        </div>
      </div>
      <div class="good-price">
        {{ parseInt(wishgood.price) }}
      </div>
      <AddToCartButton></AddToCartButton>
    </div>
    <div v-if="$store.state.display_width > 767" class="add-to-bascet">
      <div
          class="add-to-bascet__close-button"
          @click="removeWishGood(wishgood.product_id)"
      >
        <img src="@/assets/img/svg/deleteWishButton.svg" alt="delete">
      </div>
      <AddToCartButton :curID="wishgood.product_id" />
    </div>
  </div>
</template>

<script>
import AddToCartButton from "@/components/UI/AddToCartButton";
export default {
  name: 'wish-item',
  components: {
    AddToCartButton
  },
  props: ['wishgood'],
  methods: {
    removeWishGood(id) {
      this.$store.commit('REMOVE_FROM_WISH_LIST', id);
    }
  }
}
</script>

<style lang="scss" scoped>
img {
  max-width: 100%;
}

.good-name, .good-price {
  font-size: $fs-wish-list-title;
}

.__content__rows {
  display: grid;
  grid-template-columns: rem(121) repeat(2, 1fr) 1.038fr;
  grid-gap: rem(50);
  align-items: center;
  padding: $pup-row-table-whishlist 0;
  border-bottom: 1px solid;
}

.add-to-bascet {
  height: 100%;
  display: flex;
  flex-direction: column;
}

.add-to-bascet__close-button {
  align-self: flex-end;
  cursor: pointer;
}

.rows__mobile {
  border-top: 1px solid;
  grid-template-columns:rem(121) 1fr !important;
}

.mobile-column {
  height: 100%;
}

.name-close {
  display: flex;
  justify-content: space-between;
}

@media (max-width: em(767, 16)) {
  .__content__rows {
    grid-gap: rem(16);
  }
  .mobile-column {
    display: flex;
    flex-direction: column;
    justify-content: space-between;
  }
}

@media (max-width: em(428, 16)) and (min-width: em(220, 16)) {
  .rows__mobile {
    grid-template-columns: calc(4.375rem + (121 - 70) * ((100vw - 13.75rem) / (428 - 220))) 1fr;
  }
  .__content__rows {
    grid-gap: calc(0.625rem + (16 - 10) * ((100vw - 13.75rem) / (428 - 220)));
  }
}
</style>
